				...
				<div class="row jarak-card">
                    ...
                    <div class="card">
                        ...
                        <div class="card-action">
        	                <a href="?page=bkr&act=del&submit=yes&no_dentitif='.$row['no_dentitif'].'" class="btn-large deep-orange waves-effect waves-light white-text">HAPUS <i class="material-icons">delete</i></a>
        	                <a href="?page=bkr" class="btn-large blue waves-effect waves-light white-text">BATAL <i class="material-icons">clear</i></a>
    	                </div>
    	            </div>
                </div>
            </div>
            <!-- Row form END -->';
            	...
            	if(isset($_REQUEST['submit'])){
                    ...
                    //jika ada file akan mengekseskusi script dibawah ini
                        ...
                        $query = mysqli_query($config, "DELETE FROM tbl_berkas_koresponden WHERE no_dentitif='$no_dentitif'");
                        $query3 = mysqli_query($config, "SELECT *FROM tbl_isi_berkas_koresponden WHERE no_dentitifi='$no_dentitif'");
                        while($row = mysqli_fetch_array($query3)){
                            $no_itemi=$row['no_itemi'];
                        if(!empty($row['file'])){

                        unlink("upload/berkas_koresponden/".$row['file']);
                        $query = mysqli_query($config, "DELETE FROM tbl_isi_berkas_koresponden WHERE no_dentitifi='$no_dentitif' and no_itemi=' $no_itemi'");

                        
                         } else {

                        //jika tidak ada file akan mengekseskusi script dibawah ini
                        $query2 = mysqli_query($config, "DELETE FROM tbl_isi_berkas_koresponden WHERE no_dentitifi='$no_dentitif' and no_itemi='$no_itemi'");
                        }
                        ...
                        if($query == true){
                            ...
                            echo '<script language="javascript">
                                    window.location.href="./admin.php?page=bkr&act=del&no_dentitif='.$no_dentitif.'";
                                  </script>';
                        }
